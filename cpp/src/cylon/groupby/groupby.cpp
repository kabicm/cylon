...
Status DistributedHashGroupBy(std::shared_ptr<Table> &table,
  ...
  RETURN_CYLON_STATUS_IF_FAILED(Project(table, project_cols, projected_table));
  ...
  if (table->GetContext()->GetWorldSize() == 1 || is_associative(aggregate_ops)) {
    RETURN_CYLON_STATUS_IF_FAILED(HashGroupBy(projected_table,
...
Status DistributedPipelineGroupBy(std::shared_ptr<Table> &table,
  ...
  // todo: find a better way to do this, rather than checking the associativity
  if (table->GetContext()->GetWorldSize() == 1 || is_associative(aggregate_ops)) {
    RETURN_CYLON_STATUS_IF_FAILED(PipelineGroupBy(projected_table, 0, agg_after_projection, local_table));
  } else {
    local_table = std::move(projected_table);
  }
  ...
  if (table->GetContext()->GetWorldSize() > 1) {
    // shuffle
    RETURN_CYLON_STATUS_IF_FAILED(Shuffle(local_table, {0}, local_table));

    // sort the table after the shuffle
    RETURN_CYLON_STATUS_IF_FAILED(Sort(local_table, 0, local_table));

    // do local distribute again.
    RETURN_CYLON_STATUS_IF_FAILED(PipelineGroupBy(local_table, 0, agg_after_projection, output));
  } else {
    output = local_table;
  }
  return Status::OK();
